...
CREATE TABLE kb_document (
    ...
    document_code   VARCHAR(50),                       -- 业务文档稳定编号
    department_id   VARCHAR(50),                       -- 文档归属部门
    confidentiality VARCHAR(20)     NOT NULL DEFAULT '机密',
    business_status VARCHAR(20)     NOT NULL DEFAULT '未知',
...
CREATE INDEX idx_graph_task_status
    ON kb_graph_task(status, created_at);
CREATE INDEX idx_graph_task_doc_id
    ON kb_graph_task(doc_id, created_at);
CREATE UNIQUE INDEX uq_graph_task_active_doc ON kb_graph_task(doc_id)
    WHERE status IN ('PENDING', 'PROCESSING');
